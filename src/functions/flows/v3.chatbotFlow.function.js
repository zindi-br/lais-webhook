const { actionStartChatbot } = require("../../actions/chatbot.action");
...
async function v3_processChatbotFlow(scope) {
...
const callChatbotFlow = async ({ chat, channel, message }) => {
    const chatFlowId = channel?.config?.chatflow?.chatflow_id;
    if (chat) {
        if (chat.sessionFlowId) {
            try {
                const sessionFlowId = chat.sessionFlowId;
                let data = {
            ...
            try {
                ...
                if (responseStartFlow.status === 200) {
                    ...
                    data.sessionId = responseStartFlow.data.sessionId;
                    const responseContinueFlow = await actionStartChatbot(chatFlowId, data);
                    return responseContinueFlow?.data
                }
            } catch (error) {
