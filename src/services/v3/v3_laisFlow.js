...
const { v3_agentsFlowInit } = require("../../functions/flows/v3.ai.flow.function");
const { handleAvaliacao } = require("../../functions/feedback.function");

const v3LaisAiFlow = async (scope) => {
    const { clienteId, fotoPerfil, session, responseChat, responseClient, message, dataWebhooks, responseChannel, nomeContato, clientePaiId, findFirstMessage, isAgentFlow } = scope;
    ...
    if (!responseChat) {
        ...
        await v3_agentsFlowInit({message, channel:responseChannel, chat:responseChat, aiAgent, isAgentFlow, cliente_id_pai: clientePaiId});
        return;
    }
